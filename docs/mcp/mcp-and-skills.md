...
For volatile setup commands, check [MCP Setup Verification Status](verification-status.md) before copying provider-specific commands.

If these terms are still fuzzy, start with [MCP and Skills Primer](mcp-primer.md) and [SKILL.md Anatomy](skills-anatomy.md).
...
For the fuller checklist, read [MCP and Skills Security](security.md).

For concrete hook examples, read [Hooks and Guardrails](../core/hooks.md).
